                    ...
                    if($due=="")
                    {
                        ...
                        if($count>0)
                        {
                                ...
                                while($rrow=mysqli_fetch_assoc($rres))
                                {
                                    ...
                                    if(is_null($p))
                                    {
                                        $sql_s="UPDATE `$dep` SET `Total`='$tt',`stfs`='$due' WHERE `regno`='$reg'";
                                        $sqli="INSERT INTO `stfs`(`name`,`regno`,`admno`,`department`,`batch`,`due`,`remark`) VALUES ('$n','$reg','$a','$d','$b','$due','$rem')";
                                        $resi=mysqli_query($con,$sqli);
                                    }
                                    else{
                                        $amont=$p+$due;
                                        $sql_s="UPDATE `$dep` SET `Total`='$tt',`stfs`='$amont' WHERE `regno`='$reg'";
                                        $sqli="INSERT INTO `stfs`(`name`,`regno`,`admno`,`department`,`batch`,`due`,`remark`) VALUES ('$n','$reg','$a','$d','$b','$due','$rem')";
                                        $resi=mysqli_query($con,$sqli);
                                    }
