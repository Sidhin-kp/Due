...
$sqlk="SELECT * FROM `stfs` where `regno`=".$id;
...
$ree=$rowi['reason'];
  $tt=$t-$du;
  $p=$c-$du;
    $sql = "UPDATE `$d` SET Total='$tt',stfs='$p' WHERE regno = '$id'";
    if($co=="0")
    {
      $nr=$ree."STFS (".$du."/-) : ".$dur."<br>";
      $sq = "UPDATE `$de` SET `collection`='$du',`reason`='$nr' WHERE `regno` = '$id'";
      $re=mysqli_query($con,$sq);
      $s="DELETE FROM stfs WHERE `due`='$du' &&`remark`='$dur' && `regno`='$id' && `date`='$da'";
