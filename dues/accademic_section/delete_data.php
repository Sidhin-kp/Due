...
$c=$row['academic_section'];
$t=$row['Total'];

$sqlk="SELECT * FROM `academic_section` where `regno`=".$id;
...
$ree=$rowi['reason'];
  $tt=$t-$du;
  $p=$c-$du;
    $sql = "UPDATE `$d` SET Total='$tt',academic_section='$p' WHERE regno = '$id'";
    if($co=="0")
    {
      $nr=$ree."Academic Section (".$du."/-) : ".$dur."<br>";
      $sq = "UPDATE `$de` SET `collection`='$du',`reason`='$nr' WHERE `regno` = '$id'";
      $re=mysqli_query($con,$sq);
      $s="DELETE FROM academic_section WHERE `due`='$du' &&`remark`='$dur' && `regno`='$id' && `date`='$da'";
