    ...
    if(isset($_POST['sub2']))
    {
        ...
        if(isset($_POST['select_box'])) 
        {
            ...
            {          
                ...
                {
                    $reg=$_POST['reg'];
                    $sql="select COUNT(*) AS usercount from `$dep` WHERE `regno`='$reg'";
                    $res=mysqli_query($con,$sql);
                    $row=mysqli_fetch_array($res);
                    $count=$row['usercount'];
                    if($count>0)
                    {
                        $sqli="SELECT COUNT(*) AS usercount from `accounts` WHERE `regno`='$reg'";
                        $resi=mysqli_query($con,$sqli);
                        $rowi=mysqli_fetch_array($resi);
                        $counti=$rowi['usercount'];
                        if($counti>0)
                        {
                            $err="Found.Now enter the details";                        
                            header('Location: detv.php?key1=' . $reg .'&key2=' .$dep);
                        }
                        else
                        {
