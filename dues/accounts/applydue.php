<?php
    ...
    if(isset($_POST["sub0"]))
    {
        ...
        else
        {
            ...
            if(is_null($p))
            {
                $sq="UPDATE `$dep` SET `Total`='$tt',`accounts`='$due' WHERE `regno`='$no'";
                $sqli="INSERT INTO `accounts`(`name`,`regno`,`admno`,`department`,`batch`,`due`,`remark`) VALUES ('$n','$no','$a','$d','$b','$due','$rem')";
            ...
            {
                $amont=$p+$due;
                $sq="UPDATE `$dep` SET `Total`='$tt',`accounts`='$amont' WHERE `regno`='$no'";
                $sqli="INSERT INTO `accounts`(`name`,`regno`,`admno`,`department`,`batch`,`due`,`remark`) VALUES ('$n','$no','$a','$d','$b','$due','$rem')";
                $resi=mysqli_query($con,$sqli);
            }
