...
$sqlk="SELECT * FROM `accounts` where `regno`=".$id;
...
$ree=$rowi['reason'];
  $tt=$t-$du;
  $p=$c-$du;
    $sql = "UPDATE `$d` SET Total='$tt',accounts='$p' WHERE regno = '$id'";
    if($co=="0")
    {
      $nr=$ree."Accounts (".$du."/-) : ".$dur."<br>";
      $sq = "UPDATE `$de` SET `collection`='$du',`reason`='$nr' WHERE `regno` = '$id'";
      $re=mysqli_query($con,$sq);
      $s="DELETE FROM accounts WHERE `due`='$du' &&`remark`='$dur' && `regno`='$id' && `date`='$da'";
