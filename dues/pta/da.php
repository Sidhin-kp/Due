                    ...
                    if($due=="")
                    {
                        ...
                        if($count>0)
                        {
                                ...
                                while($rrow=mysqli_fetch_assoc($rres))
                                {
                                    ...
                                    {
                                        $re=$r1.$rem;
                                        $sql_s="UPDATE `$dep` SET `Total`='$tt',`pta`='$due' WHERE `regno`='$reg'";
                                        $sqli="INSERT INTO `pta`(`name`,`regno`,`admno`,`department`,`batch`,`due`,`remark`) VALUES ('$n','$reg','$a','$d','$b','$due','$rem')";
                                        $resi=mysqli_query($con,$sqli);
                                    }
                                    else{
                                        $amont=$p+$due;
                                        $sql_s="UPDATE `$dep` SET `Total`='$tt',`pta`='$amont' WHERE `regno`='$reg'";
                                        $sqli="INSERT INTO `pta`(`name`,`regno`,`admno`,`department`,`batch`,`due`,`remark`) VALUES ('$n','$reg','$a','$d','$b','$due','$rem')";
                                        $resi=mysqli_query($con,$sqli);
                                    }
