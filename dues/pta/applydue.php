<?php
    ...
    if(isset($_POST["sub0"]))
    {
        ...
        else
        {
            ...
            if(is_null($p))
            {
                $sq="UPDATE `$dep` SET `Total`='$tt',`pta`='$due' WHERE `regno`='$no'";
                $sqli="INSERT INTO `pta`(`name`,`regno`,`admno`,`department`,`batch`,`due`,`remark`) VALUES ('$n','$no','$a','$d','$b','$due','$rem')";
            ...
            {
                $amont=$p+$due;
                $sq="UPDATE `$dep` SET `Total`='$tt',`pta`='$amont' WHERE `regno`='$no'";
                $sqli="INSERT INTO `pta`(`name`,`regno`,`admno`,`department`,`batch`,`due`,`remark`) VALUES ('$n','$no','$a','$d','$b','$due','$rem')";
                $resi=mysqli_query($con,$sqli);
            }
