...
$sqlk="SELECT * FROM `pt` where `regno`=".$id;
...
$ree=$rowi['reason'];
  $tt=$t-$du;
  $p=$c-$du;
    $sql = "UPDATE `$d` SET Total='$tt',phy_edu='$p' WHERE regno = '$id'";
    if($co=="0")
    {
      $nr=$ree."Library (".$du."/-) : ".$dur."<br>";
      $sq = "UPDATE `$de` SET `collection`='$du',`reason`='$nr' WHERE `regno` = '$id'";
      $re=mysqli_query($con,$sq);
      $s="DELETE FROM pt WHERE `due`='$du' &&`remark`='$dur' && `regno`='$id' && `date`='$da'";
      $r=mysqli_query($con,$s);
    }
    else
    {
      $ct=$co+$du;
      $nr=$ree."NSS (".$du."/-) : ".$dur."<br>";
