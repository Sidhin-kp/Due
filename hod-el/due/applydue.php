<?php
    ...
    {
        $reg=$_GET['data'];
        $sq="SELECT `name` from `gptc_el` where `regno`='$reg'";
        ...
        $ro=mysqli_fetch_assoc($qry);
                ...
                {
                    $sql="SELECT COUNT(*) AS usercount FROM `gptc_el` WHERE `regno`='$reg'";
                    $res=mysqli_query($con,$sql);
                    $row=mysqli_fetch_array($res);
                    $count=$row['usercount'];
                    if($count>0)
                    {
                        $ssql="SELECT * FROM gptc_el WHERE regno='$reg'";
...
<html>
    ...
    <?php
            ...
            $no=$_GET['data'];
            $sql="SELECT `name` from `gptc_el` WHERE `regno`='$no'";
            $res=mysqli_query($con,$sql);
            $row=mysqli_fetch_array($res);
            $n=$row['name'];
