        ...
        if(isset($_POST['sub2']))
        {
            ...
            {          
                ...
                else
                {
                    ...
                    {
                        $sql="select COUNT(*) AS usercount from `gptc_ct` WHERE `batch`='$bt'";
                        ...
                        if($count>0)
                        {
                            ...
                            $ssql="SELECT regno FROM gptc_ct WHERE batch='$bt'";
                            $rres=mysqli_query($con,$ssql);
                            while($rrow=mysqli_fetch_assoc($rres))
                            {
                                $reg=$rrow['regno'];
                                $sqli="SELECT * FROM gptc_ct WHERE regno='$reg'";
                                ...
                                if($due1!="")
                                {
                                    ...
                                    if(is_null($t1))
                                    {
                                        $ssqli="SELECT Total FROM gptc_ct WHERE regno='$reg'";
                                        $rresi=mysqli_query($con,$ssqli);
                                        $rrow1=mysqli_fetch_assoc($rresi);
                                        $t4=$rrow1['Total'];
                                        $tt=$t4+$due1;
                                        $sql_s="UPDATE `gptc_ct` SET `Total`='$tt',`swlab`='$due1' WHERE `regno`='$reg'";
                                        $res_s=mysqli_query($con,$sql_s);
                                        $sqli="INSERT INTO `ctswlab`(`name`,`regno`,`admno`,`department`,`batch`,`due`,`remark`) VALUES ('$n','$reg','$a','CT','$b','$due1','$rem1')";
                                        $resi=mysqli_query($con,$sqli);
                                        if($res_s)
                                        {
                                            if($resi)
                                    ...
                                    else
                                    {
                                        ...
                                        $amount=$due1+$t1;
                                        $tt=$t4+$due1;
                                        $sql_s="UPDATE `gptc_ct` SET `Total`='$tt',`swlab`='$amount' WHERE `regno`='$reg'";
                                        $res_s=mysqli_query($con,$sql_s);
                                        $sqli="INSERT INTO `ctswlab`(`name`,`regno`,`admno`,`department`,`batch`,`due`,`remark`) VALUES ('$n','$reg','$a','CT','$b','$due1','$rem1')";
                                        $resi=mysqli_query($con,$sqli);
                                        if($res_s)
                                ...
                                if($due2!="")
                                {
                                    if(is_null($t2))
                                    {
                                        ...
                                        $t4=$rrow1['Total'];
                                        $tt=$t4+$due2;
                                        $sql_s="UPDATE `gptc_ct` SET `Total`='$tt',`hwlab`='$due2' WHERE `regno`='$reg'";
                                        $res_s=mysqli_query($con,$sql_s);
                                        $sqli="INSERT INTO `cthwlab`(`name`,`regno`,`admno`,`department`,`batch`,`due`,`remark`) VALUES ('$n','$reg','$a','CT','$b','$due2','$rem2')";
                                        $resi=mysqli_query($con,$sqli);
                                        if($res_s)
                                        {
                                            if($resi)
                                    ...
                                    else
                                    {
                                        ...
                                        $amount=$due2+$t2;
                                        $tt=$t4+$due2;
                                        $sql_s="UPDATE `gptc_ct` SET `Total`='$tt',`hwlab`='$amount' WHERE  `regno`='$reg'";
                                        $res_s=mysqli_query($con,$sql_s);
                                        $sqli="INSERT INTO `cthwlab`(`name`,`regno`,`admno`,`department`,`batch`,`due`,`remark`) VALUES ('$n','$reg','$a','CT','$b','$due2','$rem2')";
                                        $resi=mysqli_query($con,$sqli);
                                        if($res_s)
                                ...
                                if($due3!="")
                                {
                                    if(is_null($t3))
                                    {
                                        ...
                                        $t4=$rrow1['Total'];
                                        $tt=$t4+$due3;
                                        $sql_s="UPDATE `gptc_ct` SET `Total`='$tt',`ccf`='$due3' WHERE `regno`='$reg'";
                                        $res_s=mysqli_query($con,$sql_s);
                                        $sqli="INSERT INTO `ctccflab`(`name`,`regno`,`admno`,`department`,`batch`,`due`,`remark`) VALUES ('$n','$reg','$a','CT','$b','$due3','$rem3')";
                                        $resi=mysqli_query($con,$sqli);
                                        if($res_s)
                                        {
                                            if($resi)
                                    ...
                                    else
                                    {
                                        ...
                                        $amount=$due3+$t3;
                                        $tt=$t4+$due3;
                                        $sql_s="UPDATE `gptc_ct` SET `Total`='$tt',`ccf`='$amount' WHERE  `regno`='$reg'";
                                        $res_s=mysqli_query($con,$sql_s);
                                        $sqli="INSERT INTO `ctccflab`(`name`,`regno`,`admno`,`department`,`batch`,`due`,`remark`) VALUES ('$n','$reg','$a','CT','$b','$due3','$rem3')";
                                        $resi=mysqli_query($con,$sqli);
                                        if($res_s)
        ...
        <div class="form">
            ...
            <table>
                    ...
                    <tr>
                        ...
                        <span class="tooltiptext">Enter the ccf lab due</span></div></td>
                    </tr>
                    <tr>
                        <td>Remarks</td>
                        <td><div class="tooltip"><input type="text" name="t6" id="cfl">
                        <span class="tooltiptext">Enter the reson for ccf lab due</span></div>
                    </td>
                    </tr>                
                </table> 
